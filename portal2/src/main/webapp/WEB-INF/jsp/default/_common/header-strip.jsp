			...
			<c:choose>
				...
				<c:otherwise>
					<span class="white">
						...
						<a	href="/${model.myEuropeanaUrl}#user-information"
							target="<spring:message code="notranslate_main_menu_myeuropeana_a_target_t" />"
						...
						<c:if test="${!empty model.user.savedItems}">
							...
							<a id="saved-items-count" href="/${model.myEuropeanaUrl}#saved-items">${fn:length(model.user.savedItems)}</a>
						</c:if>

						<c:if test="${!empty model.user.savedSearches}">
							&nbsp;|&nbsp;
							<spring:message code="SavedSearches_t" />:
							<a id="saved-searches-count" href="/${model.myEuropeanaUrl}#saved-searches">${fn:length(model.user.savedSearches)}</a>
						</c:if>

						<c:if test="${!empty model.user.socialTags}">
							&nbsp;|&nbsp;
							<spring:message code="SavedTags_t" />:
							<a id="saved-tags-count" href="/${model.myEuropeanaUrl}#saved-tags">${fn:length(model.user.socialTags)}</a>
						</c:if>
