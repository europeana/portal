...
<c:if test="${!empty timespan.labels && (inContext == 1 || !timespan.showInContext)}">
  <c:if test="${inContext == 1}">
    ...
    <div class="contextual-header" id="${concept.htmlId}">${title}&nbsp;<span class="smaller">(</span><a class="more-info smaller"><spring:message code="enrichment_category_more_info_t" /></a><span class="smaller">)</span>
    </div>
  </c:if>
