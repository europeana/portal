...
STIMap.prototype = {
    ...
    initialize: function(containerId,toolbarStyle){
        ...
        if( STIStatic.popups ){
        	this.popup = new STIMapPopup(this.core,this.mapWindow);
        }
    ...
    setBaseLayers: function(){
		...
		if( !STIStatic.commonMapTools ){
			...
			this.mapsMenu.style.visibility = "hidden";
			var maps = document.createElement("dl");
			var map = this;
			var addEntry = function(layer){
				var entry = document.createElement("dt");
				entry.innerHTML = layer.name;
				entry.onclick = function(){
					map.openlayersMap.setBaseLayer(layer);
					map.layerSwitcherControl.src = STIStatic.path+"globe_0.png";
					map.mapsMenu.style.visibility = "hidden";
				};
	...
	initializeMapTools: function(){
		...
		if( !STIStatic.commonMapTools ){
    		...
    		this.layerSwitcherControl = document.createElement("img");
    		this.layerSwitcherControl.src = STIStatic.path+"globe_0.png";
    		this.layerSwitcherControl.title = "Choose from different map types";
			mapTools.push(this.layerSwitcherControl);
			this.layerSwitcherControl.onclick = function(){
				if( map.mapsMenu.style.visibility == "hidden" ){
		    		map.layerSwitcherControl.src = STIStatic.path+"globe_2.png";
		    		map.mapsMenu.style.visibility = "visible";
		...
		if( STIStatic.countrySelect || STIStatic.polygonSelect || STIStatic.circleSelect ){
			...
			this.navigationControl.src = STIStatic.path+"hand_2.png";
			this.navigationControl.title = "Navigate map: click and hold left mouse-button, then move mouse to change the viewport";
		...
		if( STIStatic.polygonSelect ){
			...
			this.polygonSelectionControl.src = STIStatic.path+"poly_0.png";
			this.polygonSelectionControl.title = "Select via freeform: add anchor points with left mouse-click, double click to finish the shape";
		...
		if( mapTools.length > 0 ){
			...
			logo.src = STIStatic.path+"space.png";
			logo.title = "Map Tools";
			mapTools.splice(0,0,logo);
			this.activate("navigate");
		}
    	return mapTools;
	},
