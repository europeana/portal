	...
	<c:when test="${true || !empty model.fullBeanView.docIdWindowPager}">
		...
		<ul id="navigation" class="navigation notranslate hide-on-phones ${navClass}">
			...
			<c:if test='${model.returnTo == "SEARCH_HTML" && model.fullBeanView.docIdWindowPager.next}'>
				<li>
					<a href="/${model.portalName}${model.fullBeanView.docIdWindowPager.nextFullDocUrl}" title="<spring:message code="Next_t" />" class="pagination-next europeana"><spring:message code="Next_t" />&nbsp;&nbsp;&gt;</a>
				</li>
			</c:if>
			...
			<c:if test='${model.returnTo == "SEARCH_HTML" && model.fullBeanView.docIdWindowPager.previous}'>
				<li>
					<a href="/${model.portalName}${model.fullBeanView.docIdWindowPager.previousFullDocUrl}" title="<spring:message code="Previous_t" />" class="pagination-previous europeana">&lt;&nbsp;&nbsp;<spring:message code="Previous_t" /></a>
				</li>
			</c:if>
		</ul>
	</c:when>
