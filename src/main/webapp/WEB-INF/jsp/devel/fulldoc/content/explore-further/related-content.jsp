...
<c:if
  test="${!empty model.fullBeanView.parent || !empty model.fullBeanView.children}">
  <h3>
    <a href="#related-content"><spring:message
        code="RelatedContent_t" /></a>
  </h3>
  <div id="related-content" class="carousel">
    <c:if test="${!empty model.fullBeanView.parent}">
      ...
      <a href="${model.fullBeanView.parent.fullDocUrl}"
        ...
        </c:choose> ${model.fullBeanView.parent.title}
      </a>
    </c:if>

    <div class="carousel-container" about="${model.document.id}">
      <c:if test="${!empty model.fullBeanView.children}">
        <ul>
          <c:forEach items="${model.fullBeanView.children}" var="child">
            <li><c:set var="relItemQuery"
                value='europeana_uri:"${model.document.id}"' /> <a
              href='/${model.portalName}/${fn:replace(child.id, "http://www.europeana.eu/resolve/", "")}.html?query=${relItemQuery}&amp;startPage=1&amp;pageId=brd'
