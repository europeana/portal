...
<h3 id="collapse-header-1"><span class="left collapse-header-text"><spring:message code='from_the_blog_t'/></span><span class="collapse-icon"></span><a class="feed-link icon-rss" href="http://blog.europeana.eu/feed/" target="_blank" title="RSS Feed"></a></h3>
...
<h4 class="show-on-phones"><a href="${entry.link}" title="${entry.title}" target="_self" class="europeana">${entry.title}</a></h4>
<a href="${entry.link}" title="${entry.title}" target="_self" class="image"><img class="responsive" src="/${model.portalName}${entry.images[0].responsiveFileNames['_1']}" alt="${entry.title}" /></a>
<h4 class="hide-on-phones"><a href="${entry.link}" title="${entry.title}" target="_self" class="europeana">${entry.title}</a></h4>
<p class="featured-text">${entry.description}"</p>
</div>
</div>
</c:if>
</c:forEach>
