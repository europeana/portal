...
<%--
 ...
 --%>
  ...
  <c:set var="timespanElOpen"><li></c:set>
  <c:set var="timespanElClose"></li></c:set>
  <c:set var="localSeparator" value=""/>
  
  
  <c:if test="${inContext == 1 && autoGeneratedTags == 0}">
  	<c:set var="timespanElOpen"><span class="contextual-body"  id="${concept.htmlId}"></c:set>
  	<c:set var="timespanElClose"></span></c:set>
  </c:if>
   ...
   ${timespanElOpen}<c:forEach items="${timespanOutputValues}"
		var="item"
		varStatus="status">${item}<c:if test="${!status.last}">; </c:if></c:forEach>${timespanElClose}
