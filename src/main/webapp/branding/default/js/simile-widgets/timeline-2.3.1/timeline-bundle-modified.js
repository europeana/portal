...
Timeline._Band.prototype._onTouchMove=function(D,A,E)
{
     if(A.touches.length == 1)
     {
        ...
        // These lines are part of the touch fix, but prevented the bottom band from scrolling (so are commented)
        //this._fireOnScroll();
...
Timeline.loadJSON=function(url,f){var fError=function(statusText,status,xmlhttp){alert('We apologize; the server was not able to return the data requested. Please try again later. (http error)'); js.console.error( url + '\n' + statusText );
};
var fDone=function(xmlhttp){ try { f(eval("("+xmlhttp.responseText+")"),url); } catch (e) { alert('We apologize; the server was not able to return the data requested. Please try again later. (json error)'); js.console.error(e); }
};
