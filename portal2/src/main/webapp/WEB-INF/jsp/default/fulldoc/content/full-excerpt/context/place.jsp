...
<c:if test="${!empty place.labels && (inContext == 1 || !place.showInContext)}">
 ...
 --%>
  ...
  <c:if test="${autoGeneratedTags == 0}">
    <c:set var="sectionContent"><a href="${searchUrl}" id="${fn:replace(place.about, '/', '.')}">
    <c:forEach items="${place.labels}" var="label" varStatus="t"><c:if test="${!t.first}">, </c:if>${label}</c:forEach>
    <c:forEach items="${place.labels}" var="label"><input type="hidden" name="placename" value="${label}"></c:forEach></a></c:set>  
