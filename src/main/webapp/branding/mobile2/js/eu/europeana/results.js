...
//	        jQuery('input#oq').attr('value',originalQ);
//	        // empty the search input field
//	        jQuery('input#query').attr('value','');
//	    });
});
...
function addTag(tagText, fullDocId, thumbnailId, objTitle, objType) {
	jQuery("#form-addtag").validate({
        rules: {tag: "required"}
    });
    if (jQuery("#form-addtag").valid()) {
...
function saveAlertObjectChanged(objUri, postTitle, postAuthor) {
    var sr = document.getElementById("msg-save-notification");
    sr.style.display = 'block';
    jQuery.ajax({
        type: "POST",
        url: "/portal/save.ajax",
        data: "className=Notification&europeanaUri=" + objUri + "&title=" + postTitle + "&author=" + postAuthor,
        success: function(msg) {
            sr.innerHTML = msgItemSaveSuccess;
//            var ss = document.getElementById("savedItemsCount");
//            var currentCount = parseInt(ss.innerHTML,10);
